...
- [x] ~~施設コードの対応表~~ → **2026-09-14 に埋めた。** 本番の `facilities` 17件を
      読み取り専用で参照し、依頼者が読み上げた施設名とコードの対応と全件一致を確認した
      （`src/data/loginId.ts`）。`sankokai`（シンフォニー）だけ ID が施設名から推測できない
- [ ] スマホ実機
...
carerecords を本番 Hosting へ出し（`carerecords.web.app`、`index-7hGmU367.js`）、
ココラ日根野の配信を実データで表示できることを依頼者が確認した。
**確認は PC のブラウザで行った。**

- [x] **App Check** — 本番でログインでき、配信を読めた。閉じられていない
- [x] **複合インデックス** — 実施一覧の読み取りが本番のルールとインデックスで通った
- [x] **実データの形** — kpi-react が生成した配信（`schemaVersion: 3`、12通・89訪問）を
      carerecords 側が受け取り、契約どおり解釈できた。モックでも seed でもない
- [ ] **スマホ実機** — **未確認**。PC のブラウザでの確認であり、この項目は残る
- [ ] **記録の保存（書き込み経路）** — **未確認**。確認後に本番の
      `facilities/*/visitRecords` を数えたが**全施設で0件**であり、
      書き込みはまだ一度も通っていない。読み取りだけが確かめられた状態である

**つまり Phase 5a のうち確認できたのは「読み取り」側だけである。**
書き込み・承認・オフラインはいずれも本番では未通過。

### 書き込み経路も本番で通った（2026-09-17）

依頼者がココラ日根野のサービス提供責任者（`祝田　佐和子` / `staffId: 1778144434934`）で
記録を1件保存し、リロード後も読み直せることを確認した。**PC のブラウザ。**

本番に書かれた実物を契約のスキーマ（`visitRecordSchema`、`SCHEMA_VERSION: 3`）に
通した結果、**`safeParse` が通過**した。

```
doc id   2026-09-17#1779844496894_27#imp_1084_vklri
status   済        schemaVersion 3     staffId 1778144434934
実績     06:30〜06:40   tasks ["排泄介助"]   noteSource template
mood     いつもと変わりなし
配信との整合  residentId・serviceCode とも一致（配信の予定は 06:30〜06:40 / 身体介護01・夜）
```

- **`visitId` に利用者氏名が入っていない**（`6e10f43` の意図どおり）
- **配信 → 記録の往復が実データで閉じた。** kpi-react が書いた配信を carerecords が
  読み、記録を書き戻し、その記録が配信の訪問と突き合わせられる

これで Phase 5a の残りは **スマホ実機** のみになった。

- [x] **記録の保存（書き込み経路）** — 本番で通過（上記）。ルールの書き込み側も通った
- [ ] **承認** — 未確認。保存した1件は `approvedBy` が空のまま

**承認も通った（2026-09-17）。** 同じ記録を承認し、本番に反映された。

```
status 済 → 完了     approvedBy 1778144434934     approvedAt 2026-09-17T01:17:04.608Z
```

契約スキーマは承認後も通過する。**Phase 5a で本番未確認なのはスマホ実機だけになった。**

なお今回は**サービス提供責任者が自分で記録し自分で承認した**ため、
`staffId` と `approvedBy` が同じ値になっている。仕組みの問題ではないが、
実施者と承認者が別人になる経路は本番でまだ通っていない。
