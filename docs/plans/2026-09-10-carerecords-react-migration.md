...
- [x] **6. 残りの画面** — TodoModal / PendingModal / ReportModal / TimelineModal / IncidentModal
- [x] **6r. reviewing-code（Phase 1a 全体）の指摘対応** — 要修正3・推奨7・提案4 を反映

### 振る舞いの変更（6r）

**1. 未承認一覧・未完了の訪問からの打刻／承認が、別の日・別の職員の記録にも効くようになった**

- 変更前: `mutateRecord` が対象の訪問を**表示中の配信**（表示中の日付 × 表示中の職員）からだけ探していた。未承認一覧は全職員・全期間を並べるため、他職員の行を選んで承認すると `notify('対象の訪問が見つかりません。')` で中断し、**1件も保存されないまま「N件を承認しました」と表示されていた**
- 変更後: 表示中の配信 → `visitRows`（日付・職員をまたぐ一覧）の順に解決する（`findTarget()`）。`VisitRow` に `facilityId` を足し、記録の生成に必要な情報を行から組めるようにした
- 影響: `approveVisit` は `Promise<boolean>` になった。呼び出し元は VisitList（結果を使わない）と PendingModal（成功件数を数える）の2箇所

**2. 打刻・承認の通知が、保存できたときだけ出るようになった**

- 変更前: `approveVisit` は `mutateRecord` の成否を見ずに末尾で `notify('承認しました（完了）')` を呼び、`stampStartAt` / `stampEndAt` は保存前に成功文言を通知していた
- 変更後: 保存が成功したときだけ通知する。`approveAllToday` と `PendingModal.approveSelected` も実際に保存できた件数を出す
- 影響: 承認は承認者を記録に残す法定要件のある操作で、承認漏れを利用者側から検知できることが要件になる

**3. 記録モーダルの実施内容が、表示どおりに保存されるようになった**

- 変更前: 新規記録では訪問介護計画のサービス内容が**チェック済みとして表示される**が、`draft.tasks` は空のままだった。チップに触れずに保存すると `tasks: []` で保存され、帳票（利用者別 月間サービス提供記録）の実施内容が空欄になっていた
- 変更後: `newRecordFor()` が `plannedTasks` を初期値として記録に入れる。画面は `draft.tasks` だけを見る
- 影響: 打刻（開始／終了）で新規に作られる記録にも計画のサービス内容が入る。legacy は予定側が `tasks` を持っていたため、これが legacy と同じ状態になる。**保存済みの記録は変換していない**（形は変わらないため移行不要）

**4. 訪問介護員は、自分の分の訪問しか取得しなくなった**

- 変更前: `adapter.listVisitRows()` をロールに関わらず引数なしで呼び、全職員・全期間を取得したうえで画面側（`TodoModal` の `effScope`）で自分の分だけを表示していた
- 変更後: `listVisitRows(scope)` の `scope` を必須にし、サービス提供責任者以上または承認権限を持つ職員は `{ kind: 'all' }`、それ以外は `{ kind: 'staff', staffId }` を渡す
- 影響: Phase 5 で `firestoreAdapter` に差し替えたとき、ヘルパーの全職員クエリがルールで弾かれて未完了の訪問が機能しなくなる、という失敗を先に潰した。**Phase 2 の Firestore ルールはこの2通りの読み方を前提に設計してよい**

**5. 簡易ログインが再読込のあとも保たれるようになった（計画書 §4 方針5 の未実装分）**

- 変更前: ログイン中の職員は React の state のみで、タブを再読込すると職員選択に戻っていた
- 変更後: `DataAdapter.getSessionStaffId()` / `saveSessionStaffId()` を境界に足し、`localStorage` の `carerecords.v2.session` に職員 ID だけを保存する。復元中は職員選択を出さない（`sessionRestoring`）
- 影響: 保存するのは ID だけなので、退職・権限変更が職員一覧に反映された時点で古い権限のまま操作できる状態が残らない。Phase 5 で Firebase Auth の永続化に置き換わる

**6. 日付・職員をまたぐ一覧の取得失敗が、0件ではなく失敗として表示されるようになった**

- 変更前: 未完了の訪問 / 未承認一覧 / 帳票 / 経過記録の4画面が `status === 'ready' ? data : []` で、取得に失敗しても「未完了の訪問はありません。本日分の記録はすべて終了しています。」「この月の記録はありません」と表示していた
- 変更後: loading / error / ready(empty) を出し分け、error には再試行を出す
- 影響: 帳票は請求突合に使うため、読めていないことを「記録が無い」と読ませない
...
**30. `DataAdapter.listVisitRows()` を追加した**

未承認一覧・未完了の訪問・帳票・経過記録は、いずれも**日付と職員をまたぐ**データが要る。配信は 1日 × 1職員 で取るため別経路になる。Phase 5 では Firestore のクエリになり、`from` / `to` / `staffId` で絞る。**ヘルパーはルール上自分の分しか読めないため、`staffId` を渡さない呼び出しはサービス提供責任者以上でのみ成立する**点を型のコメントに残した。

**31. `Modal` に `width` を追加した**

legacy は画面ごとに `.modal` へインラインで `width:min(Npx,100%)` を指定していた（未完了・未承認 1060px / 帳票 1000px / 利用者マスタ 980px / 経過記録 860px / ヒヤリハット 820px / 記録 既定680px）。これを写し忘れたところ、**行のグリッドにある `1fr` 列が潰れて表示が崩れた**（未完了の3列目が 298px → 22px）。ブラウザで計測して気づいた。

**32. CSV 出力・AI 生成・音声入力・記載チェックは通知のみにした**

いずれも Phase 1b の対象。マークアップは CSS 適合のため配置してある。

**33〜36. reviewing-code（Phase 1a 全体）の指摘対応で入れた変更**

- **`src/domain/vocabulary.ts` を新設した。** 実施内容の14項目が `data/mock.ts` と `RecordModal.tsx` に二重定義されていた（`MOCK_TASKS` は export されたまま誰も使っていなかった）。実施内容は Phase 1b の記載チェックが計画の `plannedTasks` と突き合わせる対象なので、片方だけ直すと突き合わせが静かにずれる。サービス種別は `serviceKindSchema.options` から取り出す形にして、契約を単一の源に保った
- **`DataAdapter` に `getSessionStaffId()` / `saveSessionStaffId()` を追加した。** 上記「振る舞いの変更 5」のとおり。UI から `localStorage` を触らせない鉄則3 を保つため、セッションもアダプタ境界の内側に置いた
- **`VisitScope` を追加し、`listVisitRows` の引数を必須にした。** 省略可能な `staffId` は「渡し忘れ」が全職員分の取得になり、その誤りが Phase 5 まで表面化しない
- **`VisitRow` に `facilityId` を追加し、`newRecordFor()` の第2引数を `RecordContext`（`Dispatch` の4項目の抜粋）に緩めた。** `Dispatch` はそのまま渡せるので既存の呼び出し2箇所は変更していない
...
**legacy の帳票は画面と CSV で母集団が違う（未対応・要判断）**

職員別の帳票は画面ではキャンセルを除外するが、CSV ではキャンセルも行として出る（`legacy/index.html:3644` vs `:3728`）。また未承認一覧の CSV は 0件で中断するのに、帳票の CSV は 0件でも空ファイルを出す。**同じ「CSV出力」ボタンで挙動が違う。** CSV は Phase 1b なので手を付けていないが、実装するときに揃えるか legacy どおりにするかの判断が要る。

**legacy の「✨ 特記事項」は再描画せず行だけ差し替える**

未完了の訪問で特記事項を作成すると、`renderTodo()` を呼ばずにその行の DOM だけを緑の完了表示に差し替える（`legacy/index.html:3427-3440`）。素直に再描画すると**ステージが -1 になって行が消え、「作成しました」の表示が一瞬も見えない**。Phase 1b で AI 生成を実装するときに、この体験をどう再現するかの判断が要る。
...
**`canApprove()` の型ガードが、否定側で `session` を `never` に狭める（局所回避）**

`canApprove(account: StaffAccount | null): account is StaffAccount` は「null を除く」ために型ガードにしてある。入力が既に非 null のとき、`!canApprove(x)` 側で `x` が `never` に狭まるため、`isSupervisor(session) || canApprove(session)` の三項の否定側で `session.staffId` が型エラーになった。**ID を先に取り出して回避している**（`CareStoreProvider.tsx` の `rowScope`）。ガード自体を変えると `approveVisit` などの null 除去が効かなくなるため触っていない。Phase 3 で `StaffAccount` を contract 側へ移すときに、判定（boolean）と null 除去を分ける形を検討する。

**サービス実施一覧は、記録が読めないときも行を「未完」として描く（未対応）**

検証で記録データを壊したときに気づいた。`VisitList` はエラー帯を出したうえで、
配信は読めているため行そのものは描画し続ける。記録が読めないので全行が「未完（登録なし）」になり、
**行だけを見ると「記録が無い」のか「記録が読めない」のかを区別できない**。
エラー帯は出ているので気づけないわけではないが、実施記録は法定文書なので、
行側にも読めていないことを出すかを Phase 1b で判断する。

**未承認一覧の一括承認は、承認1件ごとに全データを取り直す（未対応）**

`approveVisit` → `saveRecord` が毎回 `reloadToken` を進めるため、N件の承認で N 回の再取得が走る。localStorage では体感できないが、Phase 5 で Firestore になると N 回のラウンドトリップになる。**バッチ保存の経路（`saveRecords(records[])`）を Phase 5 の差し替え時に検討する。** 正しさの問題ではないため今回は手を付けていない。
...
Phase 1a（ステップ1〜6）と、reviewing-code の指摘対応（6r）を対象に検証した。
実動作は Vite の dev server（http://localhost:5173）を headless Chromium で操作し、
**画面の表示ではなく `localStorage` に保存された内容を根拠にしている**。
「画面上は成功して見えるが保存されていない」種類の欠陥を対象にしたため。

### 検証項目

- [x] **未承認一覧から他職員の「済」記録を一括承認すると、実際に保存されて「完了」になる**
  - サ責（中山 理恵）でログインし、未承認一覧で担当が自分以外の行だけを選択して承認。
    未承認バッジが 21 → 11 に減り、`carerecords.v2.visitRecords` の s002 / s003 の該当記録が
    `status: '完了'` / `approvedByName: '中山 理恵'` になった
- [x] **表示中の日付と違う日の訪問を「未完了の訪問」から打刻でき、保存される**
  - 表示は 2026-09-10 のまま、明日（2026-09-11）の「対応中」の行で終了を打刻。
    `2026-09-11-s001-01` が `actualEnd: '06:27'` / `status: '済'` で保存された。
    通知も「打刻 22:40 は予定枠外のため 06:27 で記録しました」と実際の値を出す
- [x] **打刻が拒否されたときに「記録しました」と表示されない**（否定的検証）
  - 明日 06:00〜06:08 の訪問で開始を打刻 → 「予定時間（06:00〜06:08）を過ぎています。
    「記録」から実績時間を入力してください」のみが出て、記録は作成されなかった
- [x] **承認権限のない職員が承認を押しても、承認されず成功通知も出ない**（否定的検証）
  - 訪問介護員（佐藤 健一）で「済」の記録の承認を押下 → 「承認権限がありません。」。
    記録は `status: '済'` / `approvedBy: null` のまま
- [x] **記録モーダルで実施内容に触れずに保存すると、訪問介護計画のサービス内容が保存される**
  - 明日の未記録の訪問を開き、チップに触れずに保存。画面のチェック5件
    （排泄介助 / 服薬確認 / 移動・移乗 / 見守り / 記録・連絡）と保存された `tasks` の5件が一致
    （並びは計画側の順序）。修正前は `tasks: []` で保存されていた
- [x] **訪問介護員は他職員のデータを取得しない**（否定的検証）
  - 未承認一覧・帳票・経過記録のボタンが出ない。職員切替の select が disabled。
    未完了の訪問は期間「すべて」でも担当職員が自分のみ、対象の select も disabled。
    **ヒヤリハットの利用者候補（`visitRows` 由来）がサ責 9件（—＋8名）に対し本人 7件（—＋6名）**で、
    画面で隠しているのではなく取得段階で絞られていることを確認した
- [x] **保存済みの記録が読めないとき、4画面が「0件」ではなく失敗として表示される**
  - `carerecords.v2.visitRecords` を壊れた JSON に置き換えて再読込。
    未完了の訪問 / 未承認一覧 / 帳票 / 経過記録のすべてが
    「保存されているデータの形式が正しくありません。」＋再試行ボタンを表示した
    （修正前は「未完了の訪問はありません。本日分の記録はすべて終了しています。」等）
- [x] **再読込してもログイン状態が保たれる**
  - ログイン後に reload → 職員選択画面は出ず、中山 理恵のまま一覧12件を表示。
    `carerecords.v2.session` は `{"staffId":"s001"}` のみを保持
- [x] **ログアウトすると保持が消え、再読込でも職員選択に戻る**（否定的検証）
  - `carerecords.v2.session` が削除され、reload 後も職員選択画面
- [x] **「最新の状態に更新」が実際に取得し直す**
  - 通知が「最新の状態に更新しています…」に変わり、再取得後も件数が一致（11件）
- [x] **利用者名を解決できない報告が「利用者未選択」と表示されない**
  - `residentId: 'r999'`（配信に居ない）の報告は「利用者ID r999」、
    `residentId: null` の報告は「利用者未選択」と出し分けた
- [x] **回帰: 記録支援設定の保存 / 統計 / 一覧 / ヒヤリハット一覧**
  - 利用者マスタで好みを保存 →「岩佐 久子 さんの記録スタイルを保存しました」、
    `carerecords.v2.recordPrefs` に `r001` の設定が入る。
    統計は「予定件数12 / 未完0 / 済11 / 完了0 / 実施時間214分」を表示。
    全操作を通じて **console エラー 0件・失敗リクエスト 0件**
- [ ] **職員一覧の取得に失敗したときの `.lg-err` 表示**
  - **未検証**: `listStaff()` はモック配列を返すだけで、失敗する条件を dev server 上で作れない。
    マークアップが `.lg-err on`（`styles.css:655` の `.lg-err.on{display:flex}`）であることは確認済み
- [ ] **`devTools.resetLocalData()`**
  - **未検証**: 開発用で UI から到達しない。`LOCAL_STORAGE_KEYS` の4キーと
    `INCIDENT_STORAGE_KEY` を消す実装であることの確認にとどまる

### 検証結果

- type check: 通過（`tsc -b --noEmit`）
- lint: 通過（`eslint .`）
- build: 通過（`tsc -b && vite build`、140 modules / 408.51 kB）
- test: **テストスクリプトが存在しない**（`package.json` は dev / build / lint / typecheck / preview のみ）。
  自動テストの導入は Phase 1a のスコープ外
...
**Phase 1a 完了（実装 2026-09-10 / レビュー指摘対応・検証 2026-09-11）**

### 最終的に実装したもの

単一HTML実装（`legacy/index.html` 4,457行）から React 19 + Vite 8 + TypeScript(strict) への移行を、全6ステップで完了した。

| ステップ | 内容 | コミット |
|---|---|---|
| 1 | 足場（legacy 退避 / ツールチェーン / CSS 729行移送 / CLAUDE.md） | `4c2d46d` |
| 2 | kpi-react との共有契約と永続化アダプタ | `8f54045` |
| 2r | reviewing-code の指摘対応 | `d55e5f9` |
| 3 | アプリシェルと簡易ログイン | `00c0795` |
| 4 | サービス実施一覧 | `0c32654` |
| 5 | 記録モーダルと利用者マスタ | `68dd9f0` |
| 6 | 未完了 / 未承認 / 帳票 / 経過記録 / ヒヤリハット | `77d7579` |
| 6r | reviewing-code（Phase 1a 全体）の指摘対応 — 要修正3・推奨7・提案4 | 本コミット |

**Phase 5 に向けて守れているもの**

- 永続化は `DataAdapter` の裏にあり、UI から `localStorage` を触っていない。`firestoreAdapter` に差し替えれば移行できる
- 契約（`src/types/contract.ts`）は Zod スキーマを単一の源とし、版と形の両方を実行時に検証する
- 配信に載せる情報は「訪問介護計画書に載る情報 = 配信する / 請求・資格に関する情報 = 配信しない」で線引きし、生活保護受給状況・減額認定・手帳の有無・生年月日・住所を含めていない
- 法定文書に属する情報に書き込み経路が無い
- ヒヤリハットは `incidentAdapter` の裏にあり、統合先の決定を先送りできる

**検証で確かめたこと（`## 6` に詳細）**

headless Chromium で dev server を操作し、画面表示ではなく `localStorage` に保存された内容を
根拠に14項目を確認した。12項目通過・失敗0・未検証2。
**未検証の2件（`listStaff()` の失敗時の `.lg-err` 表示 / 開発用の `resetLocalData()`）は
要件そのものではなく、発生条件を作れない副次確認であるため、依頼者の判断で `done` とした。**

**legacy から機能が増えた箇所**

- モーダルのフォーカス管理・キーボード操作・aria（legacy は未実装）
- フォームのフィールド単位のエラー表示と、失敗しても入力を失わない保存
- 契約の実行時検証と、破損した記録の部分読み出し
- 打刻・承認・保存の通知を、保存できたときだけ出す（legacy は DOM を直接書き換えて成功を表示していた）
- 訪問介護員が他職員のデータを取得しない（legacy はロールに関わらず `db` 全体を読んでいた）
- 簡易ログインの永続化（再読込で職員選択に戻らない）

### 残課題

**Phase 1b（別計画書）**

- ドメインロジックの移植: 記載チェック 474行 / 特記事項の一括作成 147行 / 帳票の集計 168行
- 音声入力 308行（Web Speech API）
- AI 特記事項作成（Cloud Functions 経由。プロバイダ未定）
- CSV 出力（画面と母集団を揃えるかの判断を含む）
- 変更履歴（`auditLog`）の表示

**判断が要る事項**

- 実施時間の合計がキャンセルの訪問も加算する（legacy の仕様）。請求の根拠になりうるため妥当性の確認が必要
- 統計の「未完＋済＋完了」が「予定件数」と一致しない（差がキャンセル件数だが表示枠が無い）
- 帳票の画面と CSV で母集団が違う
- ヒヤリハットの統合先（incident-report に寄せるか）

### 次にやるとよいこと

**Phase 2（Firestore ルールの作り直し）は carerecords と独立して進められる。** 現行の kpi-react は `allow read, write: if request.auth != null` で全開放されており、**ヘルパーアカウントを1つでも作った瞬間に全施設の全データが読み書き可能になる。** Phase 3 以降の前提であり、最も重い作業でもあるため、Phase 1b と並行して着手するのが望ましい。
