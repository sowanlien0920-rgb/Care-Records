...
本番へ配置し、PC のブラウザで配信の表示までを確認した
（`2026-09-14-carerecords-phase5a-firestore.md` の「本番接続での確認」）。
**Phase 5b の範囲はまだ1つも確かめられていない。**

- [ ] **IndexedDB が使えない端末での警告** — 未確認。プライベートブラウズの実機が要る
- [ ] **圏外での保存と `未送信` の表示** — 未確認。本番の `visitRecords` は0件であり、
      オンラインでの保存すら通っていない
- [ ] **セッションをまたいだ再送の突き合わせ**（`2026-09-15-carerecords-offline-carryover.md`）
      — 未確認。控えが作られる経路に入っていない

オフラインの確認は、**先に本番でオンラインの保存が1件通ること**が前提になる。
