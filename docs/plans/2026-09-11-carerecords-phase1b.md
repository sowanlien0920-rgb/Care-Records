...
### ステップ3の突き合わせ結果（`parity-checker`）

**定型文の本体は legacy と完全一致**（legacy の `localNote()` を抽出して Node で実行し、
過去記録依存を固定したうえで **4,000 ケースのランダム比較 → 不一致0件**）。
DOM の並び・id・class・`<option>` の value・文言もすべて一致。指摘5件のうち4件を直した（`9b52ba3`）。

- **直した**: `collect()` 相当の整形（`note` / `memo` / バイタルの trim、様子の既定値）を保存・生成・記載チェックの前に通す
- **直した**: 閉じるときに記載チェック結果・元に戻す・バッジ・文体/分量を捨てる（legacy は `openModal` で初期化）
- **直した**: 出所バッジの（要確認）／（承認済）を保存済みの記録から決める
- **直した**: `getPrefs` の失敗を握る（握らないと ✨ が二度と押せなくなる）
- **直さない**: 下書き（`edit`）がモーダルを閉じても残る点は Phase 1a 由来。legacy は破棄する

### ステップ4の内容（2026-09-11）

| ファイル | 変更 |
|---|---|
| `src/features/visitList/BulkNoteModal.tsx` | **新規（計画書の一覧に無かった。依頼者の判断で追加）。** legacy の `#bulkMask`（`:989-1046`）を移植 |
| `src/features/visitList/VisitList.tsx` | 「✨ 特記事項を一括作成」から `BulkNoteModal` を開く |
| `src/features/todo/TodoModal.tsx` | 様子の保存（`onChange`）／メモの保存（`onBlur`）／行内の ✨ 特記事項 |
| `src/store/CareStoreProvider.tsx` | 職員一覧の取得を `reloadToken` から分離（下記「実装中に気づいた点」4） |

**動作確認**（headless ブラウザ、コンソールエラー0）:
`.todo-row` の直下が span 6個 + `.todo-inline`（legacy と同じ7要素）。様子は8択で選択すると保存され、
メモは打鍵中は保存されず（行が消えず）blur で保存される。行内 ✨ で生成・保存・`noteSource='template'` まで通り、
トーストが「作成しました（記載チェックで1件の要修正）」になる。
一括作成は11件を順に処理し、進捗バーが 100% まで進み、`✓`・`定型文`・要修正件数のチップと
サマリ「作成 11件 ／ 要修正の指摘 4件」が出る。

### ステップ5の内容（2026-09-11）

| ファイル | 変更 |
|---|---|
| `src/types/speech.d.ts` | **新規。** `SpeechRecognition` まわりの最小宣言（TS 5.9 の `lib.dom` に無い） |
| `src/hooks/useSpeechInput.ts` | **新規（ディレクトリごと）。** トグル・自動再開・中間結果・カーソル位置への挿入・句点の補い・エラー文言 |
| `src/features/record/RecordModal.tsx` | 特記事項（textarea）とメモ（input）の2箇所に接続。`#micLive` を `.fld` と `#lintBox` の間に追加 |
| `src/features/todo/TodoModal.tsx` | 行内メモに接続。`#tLive` を `.plist` の外に追加 |

**動作確認**（headless ブラウザ、コンソールエラー0）:
`.sec` の直下が `h3 → #profBar → .aibar → .memowrap → .fld → #micLive → #lintBox → #aiHint` で legacy と一致。
未完了一覧も `.plist` の外に `#tLive`、`.memowrap > input.memo + button.micmini` で一致。
押すと `.micbtn` に `rec` が付いてラベルが「■ 停止」になり、`.mic-live` に `on` が付く。
純粋関数は dev server 経由で直接呼んで確認した（句点の補い3ケース、textarea は区切りなし、
input は `／`、直前が `。` なら区切りなし）。

**検証できていないこと**: 認識結果が実際に入力欄へ入る経路は**実機でしか確認できない**。
headless ブラウザは `webkitSpeechRecognition` を持つが音声デバイスが無く、
テストから認識器を差し替えることもできなかった（patchright の `evaluate` は
ページとは別の世界で動くため、`window` への細工がアプリに届かない）。計画書のリスク3のとおり。

### ステップ4・5の突き合わせ結果（`parity-checker`）

一括作成の絞り込み5条件・DOM・文言・進捗・中止・サマリ、未完了一覧の行構造と様子8択、
音声入力の設定・トグル・区切り文字・句点・エラー文言4種・`.mic-live` の位置は**すべて一致**。
指摘のうち利用者に見えるもの6件を直した。

- **直した（最重要）**: 記録モーダル・未完了一覧を閉じても録音が止まらなかった。
  legacy は `closeModal` の先頭で `micStop` する（`:1931`）。閉じた画面の下書きに
  認識結果が入り続け、マイクも回りっぱなしになる状態だった
- **直した**: 別のマイクに切り替えたとき、止めたはずの認識器が `onend` で復活しうる。
  停止時にハンドラを外すようにした
- **直した**: 一括作成で「実績時間が未入力の予定も対象にする」を選ぶと、記録がまだ無い訪問が
  全件「失敗」になっていた。保存側と同じく配信から初期値を作る
- **直した**: 一括作成の対象一覧が、取得中・取得失敗を「条件に合う記録はありません」と出していた
- **直した**: 未完了一覧のメモを `trim()` せずに保存していた（✨ 経路とで値が変わる）
- **直した**: 1行で生成中に全行の ✨ が押せなくなっていた（legacy は押した行だけ）
- **直さない（legacy と一致・軽微）**: 作成中に条件の select を `disabled` にする／条件変更で前回のサマリも消す／保存失敗時にストアのトーストが件数分出る

**未移植として残ったもの（Phase 1a からの積み残し。ステップ4の範囲外）**
- 未完了一覧の行内「予定どおり」ボタン（legacy `:3300` / `:3405-3417`）と、
  フッターの「予定どおりで確定」一括処理（`:1387` / `:3367-3383`）。`fixAsPlanned` 相当が `src/` に無い
- 未完了一覧の開始・終了のトースト文言が一覧側と共通になっている（legacy は画面ごとに違う）
- 未完了一覧の実施内容が、記録が無い行で空になる（`VisitRow` は計画の `plannedTasks` に倒す）

### ステップ6の内容（2026-09-11）

| ファイル | 変更 |
|---|---|
| `src/domain/aggregate.ts` | **新規。** `minutesOf()` / `hm()` / `userReport()` / `staffReport()` / `summaryReport()` |
| `src/utils/csv.ts` | **新規。** `toCsv()`（BOM + CRLF + 全フィールドをクォート）/ `downloadCsv()` |
| `src/features/report/ReportModal.tsx` | 集計を `aggregate.ts` に移し、帳票3種の CSV を接続 |
| `src/features/visitList/VisitList.tsx` | 日次 実施記録 CSV（15列） |
| `src/features/approval/PendingModal.tsx` | 未承認一覧 CSV（12列）＋ 記載チェックの統計・行チップ・一括承認の確認文面 |
| `src/features/timeline/TimelineModal.tsx` | 提供分数を `minutesOf()` に一本化 |
| `src/features/incident/IncidentModal.tsx` / `shell/Header.tsx` | 残るスタブの通知文を実態に合わせた |

**動作確認**（headless ブラウザ、コンソールエラー0）:
- **集計の切り出し前後で帳票3種の表示が完全一致**（画面テキストを全文比較。リスク1への対処）
- CSV 5種（日次 / 未承認 / 帳票3種）をすべてダウンロードして中身を確認した。
  列名・順序は legacy と一致、BOM あり、改行 CRLF、全フィールドがクォート済み、
  ファイル名も `実施記録_中山 理恵_2026-09-11.csv` / `未承認一覧_2026-09-11.csv` /
  `サービス提供記録_岩佐 久子_2026-09.csv` / `月間実績_中山 理恵_2026-09.csv` /
  `月間実績集計_2026-09.csv` と legacy の規則どおり
- **画面と CSV の数字が一致**（岩佐 久子：11回／82分／キャンセル1／未承認7）

### reviewing-code の結果と対応（2026-09-11）

`code-reviewer` にドメイン層・ストア・契約と UI 層を分けて読ませた。**要修正4件・推奨8件・提案9件。**
要修正4件と、利用者に見える推奨3件を直した（依頼者の判断で範囲を確定）。

#### 振る舞いの変更

| # | 変更前 | 変更後 |
|---|---|---|
| 1 | 自動修正が語幹＋活用語尾をまとめて置換し、**否定形が肯定文に化けていた**（「薬を飲ませていません」→「服薬の確認を行いましたいません」） | 置き換えても文意が反転しない活用形にだけ `fix` を付ける。該当しない語形では**ボタンを出さない**（Q11 の「押しても効かない」も同時に解消） |
| 2 | 一括作成が**承認済み（完了）の記録も対象**にしていた。承認権限の無い職員が本文を差し替えても承認印が残る | 対象から `完了` を外した。承認済みを作り直すときは、権限を検査する記録モーダルから行う |
| 3 | 音声入力・定型文生成のコールバックが押した時点の下書きを閉包しており、**録音中に実績時刻やバイタルを入れると次の確定結果で巻き戻っていた** | 下書きの更新をすべて関数型更新にした（`setDraft`）。後から届く結果が他の項目を上書きしない |
| 4 | 録音の停止を閉じる導線ごとに書いていたため、未完了一覧から「記録」を開く経路で**マイクが回り続けていた** | `useSpeechInput(visible)` の cleanup で止める。「見えなくなったら止まる」を1箇所の不変条件にした |
| 5 | CSV・印刷が、取得中・取得失敗を「出力するデータがありません」と返していた | 取得状態を先に見て、別の文言で中断する。印刷ボタンは `ready` 以外で `disabled` |
| 6 | 一括作成の実行中、保存のたびに対象一覧が「読み込んでいます…」へ差し替わっていた | 実行中は掴んだ一覧を出し続ける |
| 7 | 一括承認に二重送信のガードが無かった（Phase 1a 由来） | `busy` ガードとボタンの `disabled`／「承認中…」表示を追加 |

**呼び出し元の確認**: `check()` 6箇所 / `updateRecordFields` 3箇所 / `useSpeechInput` 2箇所。
**保存済みデータへの影響なし**（形も契約も変えていないので `migrating-database` は不要）。
**回帰確認**: 帳票3種の表示がレビュー前と完全一致。記載チェック・定型文生成・様子/メモの保存・行内 ✨ も通し確認した。

#### 今回は直さず残した推奨5件（Phase 1c 以降）

- `updateRecordFields` が read-modify-write で、同じ記録への連続更新が競合しうる（Phase 5 の Firestore で表面化する）
- 定型文が引用する計画の版と `carePlanVersion` が食い違いうる（Phase 4 で配信が実データになると静かに壊れる）
- v1 の記録が `unreadable` として残り続け、警告バナーが消えない。版上げと保存領域の作り直しが不可分であることを手順として残すべき
- `getPrefs` の失敗が3経路で無言のまま既定の文体に落ちる（生成文は記録として残るのに、設定が効かなかった痕跡が無い）
- `RecordModal` のキー付き state 6個の重複と、635行の分割（`refactoring-code` の領分）

### 計画から外れた点

**-3. 0件時の通知文は legacy の文言を使った。**
計画書の決定3は `notify('出力できる記録がありません')` としていたが、legacy の日次・未承認一覧は
`出力するデータがありません`（`:1835` / `:3572`）と出す。**0件で中断する**という Q5 の要求は
満たしたうえで、文言は legacy に合わせた（鉄則4）。帳票 CSV は legacy に0件ガードが無いので、
この文言を新たに適用した箇所になる。

**-2. ヒヤリハットの「発生時のメモ」への音声入力は見送った。**
計画書は音声入力の対象を4箇所としていたが、**その入力先である `#iMemo` が Phase 1a で
移植されておらず、`incidentSchema`（`local.ts:65-77`）にも `memo` が無い**。
欄を足すとデータ構造の変更（既存のヒヤリハット報告が検証に落ちる）になり、
唯一の利用先だった AI 報告書作成は Q1 で対象外。**接続先が無いため3箇所で完了とする。**
ヒヤリハットの AI 報告書を扱う計画で、欄ごと追加するのが筋になる。

**-1. `BulkNoteModal.tsx` を新設した（依頼者の判断）。**
計画書の変更対象ファイル一覧に一括作成のコンポーネントが無かったが、legacy の一括作成は
独立モーダル（条件4つ・進捗バー・対象一覧・中止）で `VisitList.tsx` には収まらず、
ステップ4の完了条件「一括作成が進捗を出し、中断できる」も満たせない。
**AI 前提の UI（「AI生成に失敗した記録は定型文で作成する」チェックボックス）は出さない**（依頼者の判断）。
...
**4. 記録を保存するたびに画面全体がアンマウントされていた（Phase 1a の不具合。依頼者の判断で修正）。**
`CareStoreProvider.tsx:100` の `staffKey` が `reloadToken` を含むため、記録を1件保存するたびに
職員一覧が loading に戻り、`session`（保存した ID と職員一覧から導く）が一瞬 `null` になっていた。
`App.tsx:26` は `session === null` を未ログインとして `StaffPicker` を返すので、
**開いているモーダルごと画面全体がアンマウントされていた**。
ステップ4で表面化した（メモを保存すると未完了一覧が閉じる／一括作成の進捗と結果が消える）。
職員一覧の取得キーを `staffToken` に分離し、`retry()` のときだけ両方を進めるようにした。
→ **Phase 1a の突き合わせでは検出できなかった種類の不具合**。保存の直後に必ず画面を閉じる
操作（記録モーダルの保存・削除）しか無かったため、誰も気づけなかった。

**5. 一括作成の対象一覧は、開始した時点の対象を掴んだまま描画する。**
毎回いまの条件で数え直すと、作成した端から「未入力のみ」の条件を外れて行が消え、
`✓` と要修正件数が誰にも見えないまま終わる。legacy も `bulkItems()` の結果を掴んだまま描画する（`:2906`）。

**6. 音声入力で入れた内容が保存されない経路が legacy にあった。**
legacy の `micInsert` は `el.value` に直接代入するため `input` / `change` が飛ばず、
未完了一覧の行内メモは**別の操作をするまで保存されない**（`:3176` / `:3359-3364`）。
React 側は「次の値をコールバックで返す」形にして通常の入力と同じ経路に載せ、
さらに**音声入力を止めた時点で保存する**ようにした。

**7. legacy はモーダルを閉じても録音が止まらない経路がある**（未完了一覧の閉じる・
ヒヤリハットの背景クリック。`:3356-3357` / `:4032`）。React はフックのクリーンアップで必ず止まる。

**8. legacy は画面と CSV で値が食い違う箇所を4つ抱えている**（`legacy-reader` の報告）。
移植では Q5 の確定に従って2つを揃え、2つは legacy のまま写した。
- 揃えた: 職員別帳票 CSV のキャンセル（legacy の CSV だけ含んでいて画面と不一致）／帳票 CSV の0件ガード
- 写した: 日次 CSV の「実施分」は無効時に空欄（他経路は 0）／未承認一覧 CSV の「要修正件数」は
  「済」以外で空欄（画面は 0）／集計の「要介護度」は画面が `—`、CSV が空欄

**9. `#aiHint` と一括作成の警告文が、移植対象外の設定画面を案内している。**
「⚙設定でClaude AIに切り替えられます」は legacy の文言そのままだが、設定画面は移植しない。
文言を変えない（鉄則4）ほうを採ったため、**存在しない導線を案内した状態で残っている**。
Phase 5 で AI を繋ぐときに設定画面ごと用意するか、文言を変えるかの判断が要る。
...
**10. 記載チェックが照らす訪問介護計画書が、kpi-react 側で未入力の項目を含む。**
`CarePlanSnapshot` の `supportPlan`（援助内容）/ `plannedTasks`（計画上のサービス内容）/
`householdSupportReason`（生活援助の算定理由）などが空のまま配信される。
**2026-09-14、依頼者の判断でこれを Phase 5 に入る前の運用課題として切り出した**（実装課題ではない。
計画書は kpi-react が正であり、carerecords からは書き込めない。鉄則4）。
詳細と決めるべきことは `../kpi-react/docs/plans/2026-09-14-dispatch-generation-phase4.md` の `## 9`。

carerecords 側の要件としては、**計画書が空のときに記載チェックを「問題なし」にしないこと**。
`## 5` の `3b`（空本文の早期 return が ng 0件になる）と同じ形の危険があり、
「計画書が未入力のため判定できない」と明示する必要がある。Phase 5 の記載チェック実装時に扱う。

## 6. Verification

検証日: 2026-09-11。`npm run dev`（実装と同じコード、`a8a32ea`）を headless ブラウザで操作して確認した。
**実装者の報告を根拠にせず、すべて自分で操作して観測した結果だけを書いている。**
記録は `localStorage` を空にした状態（= `resetLocalData()` 後と同じ）から始めた。

### 検証項目

**契約の版上げ（Q2 / Q7・ステップ1）**

- [x] `SCHEMA_VERSION` が 2 で保存され、「読み出せない記録が N 件あります」が出ない
  - 空の `localStorage` から起動 → 記録45件が再生成され、全件 `schemaVersion: 2`。警告バナーなし。コンソールエラー0
- [x] 記録が `mood` / `memo` / `noteSource` を持つ
  - 保存済みレコードのキーに3つとも存在することを確認

**記載チェック（確定(1) / Q4 / Q10 / Q11・ステップ2）**

- [x] 本文が空なら warn 1件だけを返し、以降の判定を行わない
  - 本文を空にして「📋 記載チェック」→ `.lv-warn` 1件（「特記事項が未入力です。…」）のみ
- [x] 違反を含む本文で、legacy と同じ順序・文言・レベルの指摘が `#lintBox` に出る
  - 6件（ng 3・warn 2 + 計画外サービス ng 1）が `LINT_RULES` の配列順どおりに描画。
    `.lintitem > .ic`（`✕` / `!`）と本文の DOM も一致
- [x] 自動修正ボタンが実際に本文を置換し、再判定で指摘が消える
  - 「薬を飲ませた」→「服薬の確認を行いました」に置換され、該当の ng が消えた
- [x] **否定**: 置換すると文意が反転する語形では自動修正ボタンを出さない（レビュー要修正1）
  - 「薬を飲ませていません」→ ng は出るが `.fixbtn` は出ない（`fix: false`）
- [x] Q11 の「legacy では押しても効かなかった語形」で実際に置換される
  - 「してあげる」→「する」に置換された
- [x] 承認時に `ng` があれば件数と全文を出し、**続行できる**（Q4。ブロックしない）
  - 「記載チェックで3件の要修正項目があります。…このまま承認しますか？」→ OK で承認が通り、
    トースト「承認しました（承認者：中山 理恵）」
- [x] **否定**: 実績が予定枠外 × サ責でも承認者でもない職員は保存できない
  - 訪問介護員（佐藤 健一）でログインし実績を 05:00 に → 「⚠ 実績時間は予定枠内で入力してください
    （開始 05:00 が予定 06:15 より前）」＋ 記載チェックが描画され、`localStorage` に保存されない。
    続けて「承認して完了」→「承認権限がありません。」
- [x] サ責は同じ操作で confirm が出て、キャンセルすれば保存されない
- [x] **同じ判定が5箇所で共有されている**（純粋関数化の必須要件）
  - 違反のある本文を1件保存し、記録モーダル3件 ／ 未承認一覧の統計「記載チェック要修正3件」 ／
    行チップ「要修正3」 ／ 一括承認の確認文面「記載チェックの要修正：3件」 ／
    CSV の「要修正件数」列 3 が**すべて一致**

**定型文生成（Q3 / Q6 / Q7 / Q8・ステップ3）**

- [x] ✨で定型文が入り、`#srcBadge` に `📄 定型文で作成` が出る
- [x] **生成しただけでは保存されない**（記録モーダルの入口。Q6）
  - 生成後に `localStorage` を見ても本文は入っていない
- [x] `元に戻す` で直前の生成前の本文に戻る（legacy の `undoStack` と同じ粒度）
- [x] `文体` / `分量` で出力が変わる
  - `long`+`plain` で締めの2ブロックが増え、文末が「〜した／〜である」に変わる
- [x] 生成して保存すると `noteSource: 'template'`、手入力して保存すると `'manual'`
  - 開き直すとバッジが `📄 定型文で作成（要確認）` になる
- [x] メモが `trim()` されて保存される（`collect()` 相当）
- [ ] **未検証**: `RecordPrefs` 取得中に `.aibtn` が `disabled` になること
  - `getPrefs` が `localStorage` 即時応答のため、開いた直後に観測しても常に `false` だった。
    CPU を 20倍スロットルしても取得待ちの状態を捕まえられていない

**未完了一覧と一括作成（Q6・ステップ4）**

- [x] 様子は `onChange` で即保存、メモは打鍵中は保存されず `onBlur` で保存される
  - 打鍵中は `localStorage` に反映されず行も消えない。blur 後に反映
- [x] 様子の選択肢が8項目（`MOOD_OPTIONS`。legacy の `MOODS` と同数）
- [x] 行内 ✨ で即保存され、行が一覧から消え、トーストで伝わる（Q6 の意図的差分）
  - 「特記事項を作成しました。内容をご確認ください」
- [x] 一括作成が進捗を出す
  - 進捗バーが 0% → 100% を10段階で進むことを MutationObserver で採取
- [x] 一括作成を**中断できる**
  - 実行中に「中止」を押す → 「中止しました（1件作成）」。残りは `○` のまま、バーは 10% で停止
- [x] 完了時にチップとサマリが出る
  - `定型文` / `指摘なし` / `要修正 1` のチップと「作成 10件 ／ 要修正の指摘 3件」
- [x] **否定**: 承認済み（完了）の記録は一括作成の対象に入らない（レビュー要修正2）
  - 「上書きして作り直す」を選んでも、12件中 完了1件・キャンセル1件を除いた10件だけが対象

**音声入力（Q9・ステップ5）**

- [x] 対象3箇所にボタンがある（記録モーダルの特記事項・メモ、未完了一覧の各行）
  - `#micNote` / `#micMemo` / `.todo-row .micmini` 11行ぶん。`#micLive` は `.fld` と `#lintBox` の間、
    `#tLive` は `.plist` の外
- [x] トグルで `.micbtn` に `rec`、ラベルが `■ 停止`、`aria-pressed=true`、`.mic-live` に `on` が付く
- [x] 中間結果は `.mic-live` にだけ出て、入力欄には入らない
- [x] 確定結果はカーソル位置に挿入され、末尾に句点が補われる
  - textarea は区切りなし（`既存の本文。` + `本日は問題ありませんでした。`）、
    input は `／` を挟む（`さいしょ／つぎのないよう。`）
- [x] モーダルを閉じると録音が止まり、`onend` の自動再開も走らない（レビュー要修正4）
  - 認識器を差し替えて `stop()` の呼び出しを観測。閉じた後の `start` は0回
- [x] **否定**: 非対応ブラウザではボタンを出さず、理由を一行出す（Q9・legacy に無い新規文言）
  - `SpeechRecognition` / `webkitSpeechRecognition` を未定義にして再読込 →
    マイクボタンが消え、「音声入力はこの端末では使えません（Chrome / Edge でご利用ください）」
- [ ] **未検証（実機が必要）**: 実際の Web Speech API で認識結果が入力欄に入ること
  - headless ブラウザには音声デバイスが無く、本物の認識器は `start()` 直後にエラーで停止する。
    **フック側の配線（トグル・挿入・句点・区切り・停止）は認識器を差し替えて確認したが、
    ブラウザの音声認識そのものに繋がる経路は確認できていない。** 計画書のリスク3のとおり
- [ ] **未検証（Phase 5 へ送る）**: スマホ3種（Android Chrome / iOS Safari / iOS Chrome）での挙動
  - **2026-09-12、依頼者の判断で「Phase 5 の配信環境で確認する」と確定した。**
    Web Speech API は secure context を要求するため、`npm run dev -- --host` で出る
    `http://192.168.x.x:5173` では確認にならない（`localhost` だけが secure context 扱い）。
    この URL をスマホで開くと `webkitSpeechRecognition` は存在するのに `start()` が
    エラーで止まり、**「この端末では使えません」の案内も出ないまま壊れて見える**ので、
    実機確認としてはむしろ誤った結論を招く。
    Q9 の帰結4（Phase 5 の配信先が HTTPS であること）に合わせ、Phase 1b では確認しない

**CSV と集計（Q5・ステップ6）**

- [x] CSV 5経路すべてが出力される
  - `実施記録_中山 理恵_2026-09-11.csv` / `未承認一覧_2026-09-11.csv` /
    `サービス提供記録_岩佐 久子_2026-09.csv` / `月間実績_中山 理恵_2026-09.csv` /
    `月間実績集計_2026-09.csv`（ファイル名は legacy の規則どおり）
- [x] 形式が5経路で共通（BOM + CRLF + 全フィールドをクォート）
  - 先頭 `EF BB BF`、`file` が「UTF-8 (with BOM), CRLF」と判定。全フィールドがクォート済み
- [x] 列が legacy と同じ
  - 日次15列（legacy `:1836` の `head` と完全一致）／未承認12列／利用者別10列／職員別8列／集計6列
- [x] **画面と CSV の数字が一致する**（Q5 の要求そのもの）
  - 日次: 画面12行・ヘッダー「実施時間 合計 214分」 ↔ CSV 12行・実施分合計 214
  - 職員別: 画面「合計 35件 / 458分 / 7時間38分」 ↔ CSV 35行・提供分合計 458
  - 利用者別（岩佐 久子）: 画面「11回 / 1時間22分 / キャンセル1 / 未承認7」「合計 82」 ↔ CSV 12行・提供分合計 82
  - 集計: 画面「8名 / 69件 / 886分」 ↔ CSV 8行・回数計 69・分計 886
  - 帳票どうしも一致（岩佐 久子の 11回・82分・キャンセル1・未承認7 が利用者別と集計で同値）
- [x] 日次 CSV に画面のフィルタが効く（Q5 の意図的差分3）
  - 「キャンセル」で絞ると画面1行・CSV も1行
- [x] 職員別帳票 CSV がキャンセルを除外する（Q5 の意図的差分1）
  - `月間実績_中山 理恵_2026-09.csv` にキャンセル行0件。画面の件数（35件）とも一致
- [x] **否定**: 0件では中断し、ファイルが出ない（Q5 の意図的差分2）
  - 「未完」で絞って0件 → ダウンロードが発生せず、「出力するデータがありません」（legacy の文言）
- [x] **集計を `aggregate.ts` へ切り出しても帳票の画面表示が変わらない**（リスク1への独立した確認）
  - 切り出し前のコミット `7a0c890` を worktree で別ポートに立て、**同一の `localStorage` を流し込んで**
    帳票3種 × 利用者8名（全10画面）のテキストを採取し、現 HEAD と `diff` → **差分0**

**回帰**

- [x] 記録の保存・承認が従来どおり動く
- [x] 保存しても画面がアンマウントされない（`## 5` の「気づいた点4」の修正が効いている）
  - 保存後もログイン状態と一覧が保たれる
- [x] 一括承認が動く（Phase 1a 機能）
  - 21件を承認 → 承認待ち0件、完了45件
- [x] 一括承認に二重送信のガードがある（レビュー推奨7）
  - 実行中はボタンが「承認中…」かつ `disabled`
- [x] 残るスタブの通知文が実態に合っている（Q1 の帰結）
  - `src/` に「Phase 1b で実装します」「ステップ6で実装します」は残っていない。
    ヒヤリハットの AI 報告書は「Phase 5（AI 接続）」、記載チェックは「別の計画」と出る
- [x] 全操作を通してコンソールエラー0（Vite の HMR WebSocket の接続失敗を除く。これは検証環境側の制約）

### 検証結果

- **type check**: 通過（`npm run typecheck`）
- **lint**: 通過（`npm run lint`。警告0）
- **build**: 通過（`npm run build`。146 modules / 443.90 kB）
- **test**: **ランナーが存在しない。** 計画書のリスク1のとおり Phase 1b の範囲外。`## 7` の残課題へ
- **検証項目**: 全47項目中 **44項目が通過、3項目が未検証（うち1項目は Phase 5 へ送ると確定）、失敗0**

### verifying-changes 内で修正したもの

**なし。** 機械的検証は初回から3つとも通過し、動作確認でも修正を要する事象は観測されなかった。

### 差し戻したもの

**なし。** 仕様・ロジックの誤りは検出されていない。

### 未検証として残ったもの（`done` にできない理由）

1. **実機での音声入力**。headless ブラウザに音声デバイスが無く、
   本物の `SpeechRecognition` は `start()` 直後にエラーで停止する。
   フック側の配線は認識器を差し替えて確認済みだが、**ブラウザの音声認識に繋がる経路は未確認**。
   → **確認手段（2026-09-12 に確定）: 依頼者が PC の Chrome か Edge で1回試す。**
   `npm run dev` で http://localhost:5173 を開き、記録モーダルの `🎤 音声入力` を押して話し、
   ①ボタンが `■ 停止` になり `.mic-live` が出る ②話している途中の文字が `.mic-live` にだけ出る
   ③確定分が特記事項に入り末尾に `。` が付く、の3点を見る。
   **スマホ3種は Phase 5 へ送る**（上記の項目を参照）
2. **`RecordPrefs` 取得中の `.aibtn` の `disabled`**。`localStorage` が即時に応答するため、
   取得待ちの状態を観測できなかった。Phase 5 で Firestore になれば観測できる

### 計画との差（`## 5` に記録済みのものを検証側でも確認した）

- **音声入力は3箇所。** 計画書のステップ5の完了条件は4箇所だが、4箇所目（ヒヤリハットの
  「発生時のメモ」）は**入力欄そのものが存在しない**ことを画面で確認した（`#iMemo` なし、
  `incidentSchema` にも `memo` なし）。`## 5` の「計画から外れた点 -2」のとおりで、
  接続先が無い以上この差は妥当と判断する
